...
<p align="center">
  <a href="https://github.com/cstueberitz/figma-agent-cli"><img src="https://img.shields.io/badge/GitHub-cstueberitz%2Ffigma--agent--cli-24292e" alt="GitHub Repo"></a>
  ...
  <img src="https://img.shields.io/badge/Multi_Assistant-Ready-blue" alt="Multi Assistant">
</p>
...
This project connects a local CLI to Figma Desktop so different assistants can operate on the open file directly:
...
- **Assistant adapters** — Keep one Figma core and add thin adapters for Codex, Claude, or local wrapper-driven agents

---

## Architecture Direction

This repo is being shaped around two layers:
...
- local or open-source assistants through wrapper scripts
- Ollama-style setups through a small bridge command instead of hard-coding one provider into the Figma core

The launcher already supports this direction with environment overrides:
...
You can also override how the launcher starts your assistant:
...
That makes the repo usable even when the target assistant needs a wrapper instead of a native Codex-style CLI.

You can manage the active assistant with:

```bash
node src/index.js assistant list
node src/index.js assistant install claude
node src/index.js assistant install ollama
...
```bash
git clone https://github.com/cstueberitz/figma-agent-cli.git
cd figma-agent-cli
npm install
npm run setup-alias
figma-ds-cli doctor
...
Then open any design file in Figma, start the `Figma Agent CLI` plugin once, and run:
...
This repo ships with an `AGENTS.md` file that gives the active assistant project-specific operating context. It covers:
...
**Want to adjust how your assistant works in this repo?** Update `AGENTS.md`. In many cases, no code change is needed.

**Example:** If you ask for a Tailwind palette, the active assistant can infer `node src/index.js tokens tailwind` from the repo instructions.
...
- **An assistant launcher** such as Codex, Claude, or a local wrapper script
...
4. Open your configured assistant in the right working context

After that, you can work from prompts instead of remembering commands.

> **Note:** `fig-start` works from any directory. The setup script saves the repo location to `~/.figma-agent-cli/config.json` and auto-migrates older config paths if present.
...
| `fig-start --here` | Start your configured assistant in the current directory and attach the figma-agent-cli repo as additional context |
| `fig-start --setup` | Change the figma-agent-cli repo path |

### First-Run Helpers

| Command | What it does |
|---------|---------------|
| `figma-ds-cli doctor` | Checks Node, your default assistant command, Figma, daemon state, and connection readiness |
...
The repository folder is `figma-agent-cli`, while the CLI executable remains `figma-ds-cli` and the launcher remains `fig-start`.
...
The included `AGENTS.md` teaches the active assistant all commands automatically. No manual required.

**Safe Mode users:** Start the Figma Agent CLI plugin each time you open Figma.
...
1. In Figma: **Plugins -> Development -> Figma Agent CLI**
