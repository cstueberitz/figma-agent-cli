...
const LAUNCHER_CONFIG_DIR = join(homedir(), '.figma-agent-cli');
const LAUNCHER_CONFIG_FILE = join(LAUNCHER_CONFIG_DIR, 'config.json');
...
const ASSISTANT_PROFILES = {
  codex: {
    ...
    displayName: 'Codex',
    command: 'codex',
    description: 'Default Codex CLI adapter using the repo root AGENTS.md file.',
    repoTemplate: '{cmd} -C "{repo}"',
    hereTemplate: '{cmd} -C "{cwd}" --add-dir "{repo}" "{instructions}"',
    profileFile: 'codex.md',
    compatFile: null,
  ...
  claude: {
    ...
    displayName: 'Claude',
    command: 'claude',
    description: 'Claude adapter that writes a repo-local CLAUDE.md companion file.',
    repoTemplate: '{cmd} -C "{repo}"',
    hereTemplate: '{cmd} -C "{cwd}" --add-dir "{repo}" "{instructions}"',
    profileFile: 'claude.md',
    compatFile: 'CLAUDE.md',
    wrapperTemplate: null
  },
  ollama: {
    id: 'ollama',
    displayName: 'Ollama Wrapper',
    command: join(homedir(), '.local', 'bin', 'figma-ollama'),
    description: 'Local wrapper around ollama run for open-source models.',
    repoTemplate: '{cmd} --repo "{repo}"',
    hereTemplate: '{cmd} --cwd "{cwd}" --repo "{repo}" --instructions "{instructions}"',
    profileFile: 'ollama.md',
    compatFile: 'OLLAMA.md',
    wrapperTemplate: join(REPO_ROOT, 'templates', 'wrappers', 'figma-ollama.sh')
  },
  generic: {
    id: 'generic',
    displayName: 'Generic Wrapper',
    command: join(homedir(), '.local', 'bin', 'figma-agent-wrapper'),
    description: 'Generic wrapper for assistants without a native repo-aware CLI.',
    repoTemplate: '{cmd} --repo "{repo}"',
    hereTemplate: '{cmd} --cwd "{cwd}" --repo "{repo}" --instructions "{instructions}"',
    profileFile: 'generic.md',
    compatFile: 'ASSISTANT.md',
    wrapperTemplate: join(REPO_ROOT, 'templates', 'wrappers', 'figma-agent-wrapper.sh')
  }
};
...
function saveConfig(config) {
    ...
    function loadLauncherConfig() {
      try {
        if (existsSync(LAUNCHER_CONFIG_FILE)) {
          return JSON.parse(readFileSync(LAUNCHER_CONFIG_FILE, 'utf8'));
        }
      } catch {}
      return {};
    }
    ...
    function saveLauncherConfig(config) {
      if (!existsSync(LAUNCHER_CONFIG_DIR)) mkdirSync(LAUNCHER_CONFIG_DIR, { recursive: true });
      writeFileSync(LAUNCHER_CONFIG_FILE, JSON.stringify(config, null, 2));
    }

    function mergeConfigs(base, updates) {
      return { ...base, ...updates };
    }

    function getAssistantProfile(profileId) {
      return ASSISTANT_PROFILES[profileId] || null;
    }

    function resolveAssistantConfig() {
      const config = loadConfig();
      const launcherConfig = loadLauncherConfig();
      const profileId = process.env.FIGMA_AGENT_PROFILE || config.assistantProfile || launcherConfig.assistantProfile || 'codex';
      const profile = getAssistantProfile(profileId) || getAssistantProfile('codex');
      const source = mergeConfigs(mergeConfigs(profile, launcherConfig), config);

      return {
        profileId: profile.id,
        displayName: process.env.FIGMA_AGENT_NAME || source.assistantName || profile.displayName,
        command: process.env.FIGMA_AGENT_COMMAND || source.assistantCommand || profile.command,
        repoTemplate: process.env.FIGMA_AGENT_REPO_TEMPLATE || source.assistantRepoTemplate || profile.repoTemplate,
        hereTemplate: process.env.FIGMA_AGENT_HERE_TEMPLATE || source.assistantHereTemplate || profile.hereTemplate,
        compatFile: source.assistantCompatFile || profile.compatFile,
        description: profile.description
      };
    }

    function syncAssistantToLauncher(config) {
      const launcherConfig = loadLauncherConfig();
      saveLauncherConfig(mergeConfigs(launcherConfig, {
        assistantProfile: config.assistantProfile,
        assistantName: config.assistantName,
        assistantCommand: config.assistantCommand,
        assistantRepoTemplate: config.assistantRepoTemplate,
        assistantHereTemplate: config.assistantHereTemplate,
        assistantCompatFile: config.assistantCompatFile
      }));
    }

    function renderAssistantProfile(profileId) {
      const profile = getAssistantProfile(profileId);
      if (!profile) throw new Error(`Unknown assistant profile: ${profileId}`);
      const base = readFileSync(join(PROFILES_DIR, 'base.md'), 'utf8').trim();
      const specific = readFileSync(join(PROFILES_DIR, profile.profileFile), 'utf8').trim();
      const generatedHeader = [
        '# Generated by figma-agent-cli',
        '',
        'This file is generated from a shared base profile plus one assistant-specific layer.',
        'Keep the section order, naming, and tone aligned with the rest of the repo when editing it.',
        ''
      ].join('\n');
...
`;
    ...
    function installAssistantProfile(profileId) {
      const profile = getAssistantProfile(profileId);
      if (!profile) throw new Error(`Unknown assistant profile: ${profileId}`);

      const installed = [];

      if (profile.compatFile) {
        const compatPath = join(REPO_ROOT, profile.compatFile);
        writeFileSync(compatPath, renderAssistantProfile(profileId));
        installed.push(compatPath);
      }

      if (profile.wrapperTemplate) {
        const wrapperName = profile.command.split('/').pop();
        const wrapperPath = profile.command.includes('/') ? profile.command : join(REPO_ROOT, 'bin', wrapperName);
        const wrapperDir = dirname(wrapperPath);
        if (!existsSync(wrapperDir)) mkdirSync(wrapperDir, { recursive: true });
        if (!existsSync(wrapperPath)) {
          writeFileSync(wrapperPath, readFileSync(profile.wrapperTemplate, 'utf8'));
    ...
    function setAssistantProfile(profileId) {
      const profile = getAssistantProfile(profileId);
      if (!profile) throw new Error(`Unknown assistant profile: ${profileId}`);

      const config = mergeConfigs(loadConfig(), {
        assistantProfile: profile.id,
        assistantName: profile.displayName,
        assistantCommand: profile.command,
        assistantRepoTemplate: profile.repoTemplate,
        assistantHereTemplate: profile.hereTemplate,
        assistantCompatFile: profile.compatFile
      });

      saveConfig(config);
      syncAssistantToLauncher(config);
      return config;
    }
...
function showQuickStart() {
  ...
  console.log();
  console.log(chalk.gray('  Learn more: ') + chalk.cyan('https://github.com/cstueberitz/figma-agent-cli\n'));
}
...
`));
  ...
  console.log(chalk.gray(`  maintained by cstueberitz • github.com/cstueberitz/figma-agent-cli\n`));
}
...
program
  ...
  .action(async (options) => {
    ...
    console.log(chalk.hex('#4ECDC4')('  Happy vibe coding! ') + chalk.gray('— cstueberitz · ') + chalk.hex('#4ECDC4')('github.com/cstueberitz/figma-agent-cli\n'));
    ...
    if (options.safe) {
      ...
      console.log(chalk.hex('#FF6B35')('\n  ┌─────────────────────────────────────────────────────┐'));
      console.log(chalk.hex('#FF6B35')('  │') + chalk.white.bold('  Setup the Figma Agent CLI plugin                 ') + chalk.hex('#FF6B35')('│'));
      console.log(chalk.hex('#FF6B35')('  └─────────────────────────────────────────────────────┘\n'));
      ...
      console.log(chalk.cyan('  → ') + chalk.white('In Figma: ') + chalk.yellow('Plugins → Development → Figma Agent CLI\n'));
...
program
  ...
  .action(async () => {
    console.log(chalk.cyan('\n🩺 Figma Agent CLI Doctor\n'));

    const health = await getConnectionHealth();
    const nodeVersion = process.version;
    const nodeMajor = parseInt(nodeVersion.slice(1).split('.')[0]);
    const assistant = resolveAssistantConfig();
    const hasAssistantLauncher = commandExists(assistant.command);
    ...
    console.log(hasAssistantLauncher ? chalk.green(`  ✓ ${assistant.command} available`) : chalk.yellow(`  ○ ${assistant.command} not found in PATH`));
...
const assistantCmd = program
  .command('assistant')
  .alias('adapter')
  .description('Manage assistant profiles, compatibility files, and launcher defaults');

assistantCmd
  .command('list')
  .description('List available assistant profiles')
  .action(() => {
    console.log(chalk.bold('\nAvailable Assistant Profiles\n'));
    for (const profile of Object.values(ASSISTANT_PROFILES)) {
      const compatLabel = profile.compatFile ? `compat: ${profile.compatFile}` : 'compat: repo AGENTS.md';
      console.log(`  ${chalk.cyan(profile.id.padEnd(10))} ${profile.displayName} — ${profile.description}`);
      console.log(chalk.gray(`             command: ${profile.command} • ${compatLabel}`));
    }
    console.log();
  });
...
assistantCmd
  .command('current')
  .description('Show the currently configured assistant')
  .action(() => {
    const assistant = resolveAssistantConfig();
    console.log(chalk.bold('\nCurrent Assistant\n'));
    console.log(`  ${chalk.cyan('profile'.padEnd(14))}${assistant.profileId}`);
    console.log(`  ${chalk.cyan('name'.padEnd(14))}${assistant.displayName}`);
    console.log(`  ${chalk.cyan('command'.padEnd(14))}${assistant.command}`);
    console.log(`  ${chalk.cyan('repo template'.padEnd(14))}${assistant.repoTemplate}`);
    console.log(`  ${chalk.cyan('here template'.padEnd(14))}${assistant.hereTemplate}`);
    console.log(`  ${chalk.cyan('compat file'.padEnd(14))}${assistant.compatFile || 'AGENTS.md'}`);
    console.log();
  });

assistantCmd
  .command('show <profile>')
  .description('Show details for a specific assistant profile')
  .action((profileId) => {
    const profile = getAssistantProfile(profileId);
    if (!profile) {
      console.log(chalk.red(`Unknown assistant profile: ${profileId}`));
      process.exit(1);
    }

    console.log(chalk.bold(`\n${profile.displayName}\n`));
    console.log(`  ${chalk.cyan('command'.padEnd(14))}${profile.command}`);
    console.log(`  ${chalk.cyan('compat file'.padEnd(14))}${profile.compatFile || 'AGENTS.md'}`);
    console.log(`  ${chalk.cyan('description'.padEnd(14))}${profile.description}`);
    console.log();
  });

assistantCmd
  .command('use <profile>')
  .description('Make an assistant profile the default for fig-start and doctor')
  .option('--install', 'Also install compatibility files for the selected profile')
  .action((profileId, options) => {
    ...
    const config = setAssistantProfile(profileId);
    let installed = [];
    if (options.install) {
      installed = installAssistantProfile(profileId);
    }

    console.log(chalk.green(`\n✓ ${profile.displayName} is now the default assistant.`));
    console.log(chalk.gray(`  command: ${config.assistantCommand}`));
    if (installed.length > 0) {
      console.log(chalk.gray(`  installed: ${installed.join(', ')}`));
    }
    console.log();
  });
...
assistantCmd
  ...
  .description('Install assistant compatibility files and make the profile current')
  .action((profileId) => {
    const profile = getAssistantProfile(profileId);
    if (!profile) {
      console.log(chalk.red(`Unknown assistant profile: ${profileId}`));
    ...
    const installed = installAssistantProfile(profileId);
    const config = setAssistantProfile(profileId);

    console.log(chalk.green(`\n✓ Installed ${profile.displayName} helper files.`));
    if (installed.length === 0) {
      console.log(chalk.gray('  No extra files were needed for this profile.'));
    } else {
      for (const file of installed) {
        console.log(chalk.gray(`  ${file}`));
      }
      console.log(chalk.gray('  generated files use the shared base profile first, then the assistant-specific layer.'));
    }
    console.log(chalk.gray(`  default command: ${config.assistantCommand}`));
    console.log();
  });
